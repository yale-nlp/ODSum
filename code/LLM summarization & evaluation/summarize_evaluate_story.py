...
class Evaluate:
    ...
    def another_rouge(path, predictions, references, model_name):
        ...
        rouge_score = metric_realization.calculate_rouge(
            ref=references, pred=predictions
        )
        # Save
    ...
    def bleurt(path, predictions, references, model_name):
        ...
        rouge_score = metric_realization.calculate_bert_score(
            ref=references, pred=predictions
        )
        # Save
    ...
    @staticmethod
    def gpt_eval(
        path, predictions, references, model_name, dataset: str, gpteval_summary_index=1
    ):
        ...
        for metric_type in metric_list:
            ...
            for index, prediction in enumerate(predictions):
                ...
                cur_prompt = prompt.replace("{{Document}}", reference).replace(
                    "{{Summary}}", prediction
                )
            ...
            for _ in range(23):
                ...
                num_list = ["1", "2", "3", "4", "5"]
                response_list = [
                    item for item in response_list if item and item[0] in num_list
                ]
    ...
    def evaluate(
        ...
        # Delete empty
        references = [
            references[index] for index, item in enumerate(predictions) if item != ""
        ]
        predictions = [
            predictions[index] for index, item in enumerate(predictions) if item != ""
        ]

        # create save directory if not exists
        if not os.path.exists(os.path.join(path, 'evaluation', model_name)):
            os.makedirs(os.path.join(path, 'evaluation', model_name))
...
Evaluate.traverse_path(
    root="QMSum", model_name="unlim", dataset="qmsum", rouge=True, bert=True,gpteval=True
)
